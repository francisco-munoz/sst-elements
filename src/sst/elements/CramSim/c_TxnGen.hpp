...
/* Element Library Info */
            SST_ELI_REGISTER_COMPONENT(c_TxnGen, "CramSim", "c_TxnGen", SST_ELI_ELEMENT_VERSION(1,0,0), "Test Txn Generator", COMPONENT_CATEGORY_UNCATEGORIZED)
    
            SST_ELI_DOCUMENT_PARAMS(
		{"maxOutstandingReqs", "Maximum number of the outstanding requests"},
		{"numTxnPerCycle", "The number of transactions generated per cycle"},
		{"readWriteRatio", "Ratio of read txn's to generate : write txn's to generate"})
            
            SST_ELI_DOCUMENT_PORTS( { "memLink", "link to memory-side components (txn dispatcher or controller)", { "c_TxnReqEvent", "c_TxnResEvent" } } )

            SST_ELI_DOCUMENT_STATISTICS(
                {"readTxnsSent", "Number of read transactions sent", "reads", 1},
                {"writeTxnsSent", "Number of write transactions sent", "writes", 1},
                {"readTxnsCompleted", "Number of read transactions completed", "reads", 1},
