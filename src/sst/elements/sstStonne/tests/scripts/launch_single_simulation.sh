...
if [ "$#" -ne 8 ]
then
    echo "Usage: ./$0 M N K MK_sparsity KN_sparsity stonne_inner_conf_file stonne_outer_conf_file stonne_gustavsons_file"
    exit 1
fi
...
cp template_gen_gustavsonsGEMM.py temporal_gen_gustavsonsGEMM.py
...
cp template_sst_stonne_gustavsons.py temporal_sst_stonne_gustavsons.py
...
sed -i "s/STONNE_GUSTAVSONS_FILE/$STONNE_GUSTAVSONS_CONF_FILE/g" temporal_*
...
# Generating matrices and addresses for gustavsons
output_cmd=$(python temporal_gen_gustavsonsGEMM.py)
address_b=$(echo "$output_cmd" | grep "Offset matrix B" | tr -d ' ' | cut -f 2 -d:)
address_c=$(echo "$output_cmd" | grep "Offset matrix C" | tr -d ' ' | cut -f 2 -d:)
     #  Replacing address in sst conf file
     sed -i "s/MATRIX_B_DRAM_ADDRESS_PARAMETER/$address_b/g" temporal_sst_stonne_gustavsons.py
     sed -i "s/MATRIX_C_DRAM_ADDRESS_PARAMETER/$address_c/g" temporal_sst_stonne_gustavsons.py
...
idle_cycles_as_gustavsons=$(cat *.txt | grep "Idle_cycles_aswitch" | tr -d , | tr -d ' ' | cut -f 2 -d: | paste -d+ -s | bc)
...
rm -f *.in
rm -f *.ini
echo "$cycles_inner $cycles_outer $cycles_gustavsons $idle_cycles_ms_inner $idle_cycles_ms_outer $idle_cycles_ms_gustavsons $idle_cycles_as_inner $idle_cycles_as_outer $idle_cycles_as_gustavsons $n_cycles_multiplying_outer $n_cycles_merging_outer $l1_hits_inner $l1_hits_outer $l1_hits_gustavsons $l1_miss_inner $l1_miss_outer $l1_miss_gustavsons"
